...
export function MainPage({ screenWidth }: { screenWidth: number }) {
    ...
    const [currentFolder, setCurrentFolder] = useState<Folder>({ _id: "", name: "All Notes", userId: "", parentFolder: "" })
    ...
    useEffect(() => {
        if (currentFolder._id) {
            console.log('Current folder ID:', currentFolder._id);
            console.log('Available folders:', folders);
            const folderDetails = folders.find(folder => folder._id === currentFolder._id);
            console.log('Found folder details:', folderDetails);
            if (folderDetails) {
    ...
    function logoClickHandler() {
        setCurrentFolder({ _id: "", name: "", userId: "", parentFolder: "" })
    ...
    return <div >
        ...
        {screenWidth < 768 ? <div className='bg-blue2 min-h-screen'>
            ...
            <div className="box-border px-[16px]">
                ...
                <div className="pt-[126px] relative mt-[10px] text-[20px] text-white font-semibold mb-[10px]">{currentFolder.name}</div>
                <Folders refresh={refresh} setRefresh={setRefresh} deleteFolderHandler={deleteFolderHandler} folders={folders} setCurrentFolder={setCurrentFolder}></Folders>
            ...
            <div className="bg-blue2 min-h-screen w-full overflow-y-auto">
                ...
                <div className={`${!sidebar ? "ml-[15%] mid:ml-[10%]" : "ml-[30%] mid:ml-[20%]"} box-border relative h-screen overflow-y-auto`}>
                    ...
                    <div className="px-[24px]">
                        <div className="mt-[33px] text-[24px] text-white font-semibold mb-[25px]">{currentFolder.name}</div>
                        <div className={`grid grid-cols-3 gap-5 ${!sidebar ? "tablet:grid-cols-3 mid:grid-cols-5 laptop:grid-cols-7" : "tablet:grid-cols-2 tablet:gap-6 mid:grid-cols-4 laptop:grid-cols-5"} tablet:gap-5 mb-[25px]`}>
                            ...
                            {folders.map((folder) => (
                                <FolderButtons refresh={refresh} setRefresh={setRefresh} deleteFolderHandler={deleteFolderHandler} setCurrentFolder={setCurrentFolder} key={folder._id} color="bgrey" icon="folder" text={folder.name} id={folder._id}></FolderButtons>
                            ))}
                        </div>
        ...
        {contentDialogBox ? <div className="fixed z-50 inset-0 flex items-center justify-center bg-blue2/90  before:content-[''] before:pointer-events-none"> <AddContent setRefresh={setRefresh} refresh={refresh} contentDialogBox={contentDialogBox} setContentDialogBox={setContentDialogBox} folder={currentFolder._id}></AddContent></div> : null}
        {folderDialogBox ?
            <div className="fixed z-50 inset-0 flex items-center justify-center bg-blue2/90 before:content-[''] before:pointer-events-none">
                <AddFolder
                    refresh={refresh}
                    setRefresh={setRefresh}
                    setFolderDialogBox={setFolderDialogBox}
                    folderDialogBox={folderDialogBox}
